...
export const Catalogo: React.FC = () => {
  ...
  const handleStartEdit = (prod: Producto) => {
    ...
    setCategoriaSelect(prod.categoria || "");
    setSubcategoriaSelect(prod.subcategoria || "");
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: "4px", maxWidth: "250px" }}>
                        {prod.ingredientes && prod.ingredientes.length > 0 ? (
                          prod.ingredientes.map(ing => (
                            <span key={ing.insumoId} className="insumo-requirement-badge">
                              {ing.insumo ? ing.insumo.nombre : `Insumo #${ing.insumoId}`}: {ing.cantidadNecesaria} {ing.insumo?.unidadMedida || ""}
                            </span>
                          ))
                        ) : (
