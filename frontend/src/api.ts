export const BACKEND_URL = (import.meta.env.VITE_BACKEND_URL as string) || (typeof window !== "undefined" ? `http://${window.location.hostname}:8000` : "http://localhost:8000");
...
export const api = {
  ...
  insumos: {
    getAll: (search?: string) =>
      request<Insumo[]>(`/insumos/${search ? `?search=${encodeURIComponent(search)}` : ""}`),
    create: (data: Partial<Insumo>) =>
      request<Insumo>("/insumos/", {
        method: "POST",
        body: JSON.stringify(data),
      }),
    update: (id: number, data: Partial<Insumo>) =>
      request<Insumo>(`/insumos/${id}`, {
        method: "PUT",
        body: JSON.stringify(data),
      }),
    delete: (id: number) =>
      request<{ message: string }>(`/insumos/${id}`, {
        method: "DELETE",
      }),
    comprar: (id: number, cantidad: number) =>
      request<Insumo>(`/insumos/${id}/comprar`, {
        method: "POST",
