...
from backend.models import (
    ...
    FactInventario, FactReserva, Insumo, ProductoInsumo
)
from backend.schemas import (
    UsuarioOut, ClienteCreate, ClienteOut, ProveedorCreate, ProveedorOut,
    ProductoCreate, ProductoOut, MesaCreate, MesaOut,
    VentaCreate, VentaEncabezadoOut, DetalleCocinaUpdate,
    InsumoCreate, InsumoOut, InsumoCompra
)
...
@app.get("/api/insumos/", response_model=List[InsumoOut])
def get_insumos(search: Optional[str] = None, db: Session = Depends(get_db)):
    query = db.query(Insumo)
    if search:
        query = query.filter(
            or_(
                Insumo.nombre.ilike(f"%{search}%"),
                Insumo.categoria.ilike(f"%{search}%")
            )
        )
    return query.all()

@app.get("/api/insumos/{id}", response_model=InsumoOut)
def get_insumo(id: int, db: Session = Depends(get_db)):
    insumo = db.query(Insumo).filter(Insumo.insumo_id == id).first()
    if not insumo:
        raise HTTPException(status_code=404, detail="Insumo no encontrado")
    return insumo

@app.post("/api/insumos/", response_model=InsumoOut)
def create_insumo(dto: InsumoCreate, db: Session = Depends(get_db)):
    insumo = Insumo(
        nombre=dto.nombre,
        categoria=dto.categoria,
...
@app.put("/api/insumos/{id}", response_model=InsumoOut)
def update_insumo(id: int, dto: InsumoCreate, db: Session = Depends(get_db)):
    insumo = db.query(Insumo).filter(Insumo.insumo_id == id).first()
    if not insumo:
        raise HTTPException(status_code=404, detail="Insumo no encontrado")
    ...
    insumo.proveedor_id = dto.proveedor_id
    insumo.estado = dto.estado
    
    db.commit()
    db.refresh(insumo)
    return insumo

@app.delete("/api/insumos/{id}")
...
@app.post("/api/insumos/{id}/comprar", response_model=InsumoOut)
def comprar_insumo(id: int, dto: InsumoCompra, db: Session = Depends(get_db)):
    insumo = db.query(Insumo).filter(Insumo.insumo_id == id).first()
    if not insumo:
        raise HTTPException(status_code=404, detail="Insumo no encontrado")
    
    from decimal import Decimal
    insumo.stock_actual = insumo.stock_actual + Decimal(str(dto.cantidad))
    db.commit()
    db.refresh(insumo)
    return insumo
...
def update_cocina_status(detalleId: int, dto: DetalleCocinaUpdate, db: Session = Depends(get_db)):
    ...
    old_status = det.estado_cocina
    new_status = dto.estado_cocina
    
    # Deduct stock if starting preparation (transitioning to preparacion from a different state)
    if new_status == "preparacion" and old_status not in ["preparacion", "listo", "completado", "entregado"]:
        if det.producto_id:
            from decimal import Decimal
            qty = det.cantidad if det.cantidad else 1
            ingredients = db.query(ProductoInsumo).filter(ProductoInsumo.producto_id == det.producto_id).all()
            for ing in ingredients:
                insumo = db.query(Insumo).filter(Insumo.insumo_id == ing.insumo_id).first()
                if insumo:
                    consumed = Decimal(str(ing.cantidad_necesaria)) * Decimal(str(qty))
                    if insumo.stock_actual is None:
                        insumo.stock_actual = Decimal("0.00")
                    insumo.stock_actual = insumo.stock_actual - consumed
                    
    det.estado_cocina = new_status
    db.commit()
