...
public class VentaController {
    ...
    public ResponseEntity<VentaEncabezado> createVenta(@RequestBody VentaCreateDto dto) {
        ...
        // Enviar comprobante al cliente por correo electrónico si está registrado y
        // tiene correo
        if (savedVenta.getCliente() != null && savedVenta.getCliente().getEmail() != null
                && !savedVenta.getCliente().getEmail().trim().isEmpty()) {
            try {
    ...
    public ResponseEntity<List<Map<String, Object>>> getHistorialVentas() {
        ...
        ventas.sort((v1, v2) -> {
            if (v1.getFechaHora() == null && v2.getFechaHora() == null)
                return 0;
            if (v1.getFechaHora() == null)
                return 1;
            if (v2.getFechaHora() == null)
                return -1;
        ...
        for (VentaEncabezado v : ventas) {
            ...
            if (v.getDetalles() == null || v.getDetalles().isEmpty()) {
                ...
                row.put("cantidad", 1);
                row.put("precio_unitario", v.getSubtotal() != null ? v.getSubtotal()
                        : (v.getTotalFinal() != null ? v.getTotalFinal() : BigDecimal.ZERO));
                row.put("subtotal_detalle", v.getSubtotal() != null ? v.getSubtotal()
                        : (v.getTotalFinal() != null ? v.getTotalFinal() : BigDecimal.ZERO));
                ...
                for (VentaDetalle d : v.getDetalles()) {
                    ...
                    BigDecimal precio = d.getPrecioUnitarioMomento() != null ? d.getPrecioUnitarioMomento()
                            : BigDecimal.ZERO;
                    ...
                    row.put("tiempo_preparacion",
                            d.getTiempoPreparacionReal() != null ? d.getTiempoPreparacionReal() : "N/A");
    ...
    public ResponseEntity<?> updateEstadoCocina(@PathVariable("detalleId") Integer detalleId,
            @RequestBody DetalleCocinaUpdateDto dto) {
        ...
        try {
            EstadoCocina oldStatus = det.getEstadoCocina();
            EstadoCocina newStatus = EstadoCocina.valueOf(dto.getEstado_cocina().toLowerCase());

            // Deduct stock if starting preparation (transitioning to preparacion from a
            // different state)
            if (newStatus == EstadoCocina.preparacion && oldStatus != EstadoCocina.preparacion
                    && oldStatus != EstadoCocina.listo) {
                if (det.getProducto() != null) {
                    Integer prodId = det.getProducto().getProductoId();
                    Integer qty = det.getCantidad() != null ? det.getCantidad() : 1;

                    List<ProductoInsumo> ingredients = productoInsumoRepository.findByProductoId(prodId);
                    for (ProductoInsumo ing : ingredients) {
                        Optional<Insumo> insumoOpt = insumoRepository.findById(ing.getInsumoId());
                        if (insumoOpt.isPresent()) {
                            Insumo insumo = insumoOpt.get();
                            BigDecimal consumed = ing.getCantidadNecesaria().multiply(BigDecimal.valueOf(qty));
                            if (insumo.getStockActual() == null) {
                                insumo.setStockActual(BigDecimal.ZERO);
                            }
                            insumo.setStockActual(insumo.getStockActual().subtract(consumed));
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body(mapError("Estado de cocina inválido: " + e.getMessage()));
        }
    }
    ...
    private void triggerWebhook(VentaEncabezado venta) {
        ...
        try {
            ...
            json.append("\"clienteNombre\":\"")
                    .append(escapeJson(
                            venta.getCliente() != null ? venta.getCliente().getNombre() : "Consumidor Final"))
                    .append("\",");
            json.append("\"mesa\":\"")
                    .append(escapeJson(
                            venta.getMesa() != null ? "Mesa " + venta.getMesa().getNumeroMesa() : "Para Llevar"))
                    .append("\",");
            ...
            if (venta.getDetalles() != null) {
                for (int i = 0; i < venta.getDetalles().size(); i++) {
                    ...
                    json.append("{");
                    json.append("\"productoNombre\":\"")
                            .append(escapeJson(d.getProducto() != null ? d.getProducto().getNombre() : "Producto"))
                            .append("\",");
                    json.append("\"cantidad\":").append(d.getCantidad()).append(",");
                    json.append("\"precioUnitario\":")
                            .append(d.getPrecioUnitarioMomento() != null ? d.getPrecioUnitarioMomento() : 0);
                    json.append("}");
